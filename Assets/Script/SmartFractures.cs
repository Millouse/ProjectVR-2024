...
public class SmartFracture : MonoBehaviour
{
    ...
    void InitSubFractures()
    {
        ...
        foreach (SubFracture cell in cells)
        {
            ...
            {
                if (hitColliders[i].GetComponent<SubFracture>() &&
                    hitColliders[i].transform.root == cell.transform.root &&
                    hitColliders[i].gameObject != cell.gameObject) // Make sure it is a sub fracture of this object
                {
                    cell.connections.Add(hitColliders[i].GetComponent<SubFracture>());
                    Debug.Log("Début de la liste de connection de la cell " + cell.name);
                    foreach (var cellConnection in cell.connections)
                    {
                        Debug.Log(cellConnection.name);
                    }
